function spillMotMinMax(farge, id) {
  ...
  /*
  Viser trekket etter hvert flytt i lisetOverFen. Henter history og putter den i en array 
  => printer ut hvilken runde det er først (i) hvis indexen er ett parrtall(som vi si at ad et er hvit sin tur) og 
  deretter hvilken trekk som er gjort
   */
  function getHistory() { 
  ...
  //Legger inn de funksjonene vi ønsker i config til sjakkspillet
  var config = {
    draggable: true,
    position: 'start',
  ...
  */
  ...
  function checkStatus(color) {
    //Setter riktig farge (c)
    var victory = document.getElementById("victory")
    var c = color;
  
    if(farge === "white")
      color === "white" ? c = "Black" : c = "White";
    //Ut ifra gamestate så setter vi tekst om hvem som vent/stalemate/draw
    if (game.in_checkmate()) {
      if(board === "myBoard") {
  ...
  */
    ...
    //Definerer hvem som spiller slik at motsatt farge kan gjøre best mulig trekk
    let c;
    farge === "black" ? c = 'w' : c = 'b'
    {
    ...
    if(id === 'myBoard') //Funker bare for spill.html
        getHistory()
  ...
  function spillMedAapning(farge, id, fenString) {
    var board = null
    var game = new Chess()  
    ...
    function onDrop (source, target) {
     // see if the move is legal
      ...
      //Etter ditt trekk så settes fenstringen inn i local storage som kan brukes til å lagre åpningen i åpninger siden
      localStorage.setItem('egenÅpning', board.position('fen'))
      sound()
    }
    ...
    //Laster inn åpningen og ser på fenstrigen hvilken turn det er. Hvis svart så gjøre tilfeldig trekk
    if(fenString) { 
      let tab = fenString.split(" ")
      if(tab[1] == 'b')
        window.setTimeout(makeRandomMove, 500)
  ...
  //Funksjon som laster en bestemt åpning
  function loadOpener(id, fen) {
    board = Chessboard(id, fen)
  }
  
  //spiller "brikke flyttet" lyd
  function sound() {
